  ...
  return (
    <div className='flex w-1/3 xl:hidden justify-start items-center'>
      <button className='border border-black w-min ms-8 rounded-md hover:scale-110 transition-all bg-white fixed'>
        <MenuIcon className='text-black' onClick={() => setIsOpen(true)} size={40}/>
      </button>
      {isOpen && (
        <div className='w-screen min-h-full h-screen max-h-screen fixed top-0 left-0 bg-white flex flex-col gap-4'>
          <div className='flex justify-between items-center w-full max-w-full p-4 sm:px-8'>
            <h1 className='uppercase text-3xl font-bold tracking-[0.7rem]'>Epimerch</h1>
            <button className='hover:scale-110 transition-all' onClick={() => setIsOpen(false)}>
              <XIcon className='text-black' size={40}/>
            </button>
          </div>
          {user && (<h2 className='p-4 sm:px-8 text-xl -mt-4'>Bonjour {user.firstname}!</h2>)}
          ...
          <div className='flex flex-col gap-8 p-4 sm:px-8 justify-center text-xl select-none'>
            <Link to='/'><span className='hover:bg-gray-100 p-2 rounded-md'>Accueil</span></Link>
            <details>
              <summary className='hover:cursor-pointer'>Été 2024</summary>
              <ul className='flex flex-col gap-4 px-4 py-4'>
                <li><Link to={'/clothes?category=shirt'}>T-Shirts</Link></li>
                <li><Link to={'/clothes?category=hat'}>Chapeaux</Link></li>
                <li><Link to={'/clothes?category=shorts'}>Shorts</Link></li>
                <li><Link to={'/clothes?category=accessories'}>Accessoires</Link></li>
                <li><Link to={'/clothes?category=other'}>Autres</Link></li>
              </ul>
            </details>
